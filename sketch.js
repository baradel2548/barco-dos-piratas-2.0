...
function preload() {
  ...
  cannonC_music = loadSound("assets/cannon_explosion.mp3")
  cannon_water_music = loadSound("assets/cannon_water.mp3")
  pirata_laugh = loadSound("assets/pirate_laugh.mp3")
}
...
function draw() {
  ...
  if(!background_music.isPlaying()){
    ...
    background_music.setVolume(0.08)
  }
  ...
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      if (collision.collided && !boats[i].isBroken) {
        isGameOver = true;
        if(!isLaughing && !pirata_laugh.isPlaying()){
          pirata_laugh.play()
          isLaughing = true
        }
        gameOver();
